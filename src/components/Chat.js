  ...
  const renderChat = (chat) => {
    ...
    if (byMe)
      return (
        <div
          key={chat.id}
          className={`bg-green-200 break-words break-all text-black max-w-[80%] w-fit p-2 rounded mb-2 self-end`}
        >
          {chat.text}
          {chat?.attachment && (
            <img
    ...
    <div className="relative flex-grow h-[85vh] bg-gray-50 rounded-md flex flex-col">
      ...
      {image && (
        <div className="absolute shadow-xl z-40 left-0 h-72 overflow-hidden bottom-16 w-full bg-white rounded-md">
          {loading && (
            <div className="flex items-center justify-center absolute z-40 self-center w-full h-full bg-gray-600 bg-opacity-60">
